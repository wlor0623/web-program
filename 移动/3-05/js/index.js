/*
 * @Author: xyl
 * @Date:   2017-11-02 09:27:54
 * @Last Modified by:   xyl
 * @Last Modified time: 2017-11-02 17:12:45
 */
// 等页面的元素和资源请求结束的时候会执行 保证元素能够获取 还要保证图片也加载完毕才能执行JS
// window.onload = function () {

// }
/*addEventListener HTML5新增的添加事件的方式 事件添加 也可以解绑 不是DOM元素的一个属性
这种方式可以同一个事件添加多次 在移动端一般使用这种方式
（移动端特有的事件滑动事件之类的事件必须使用这种方式添加）*/
// 页面加载事件
window.addEventListener('load', function() {
    //JS代码有3个特效 而且特效之间没有关联 最好把3个特效的代码分开 用函数分开
    // 在load事件里调用3个特效的函数
    searchEffect();
    countDownEffect();
    slideEffect();
